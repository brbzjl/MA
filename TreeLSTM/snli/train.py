...
if args.spinn:
    ...
    transitions = datasets.nli.ShiftReduceField()#ransitions = None
answers = data.Field(sequential=False)
vec = vocab.Vectors('glove.840B.300d.txt', '../../glove.840B.300d')

train, dev, test = datasets.SNLI.splits(inputs, answers, transitions)

print(train[0].__dict__.keys())
print(train[0].label[:3])
print(train[0].premise[:3])
print(train[0].hypothesis[:3])
...
for epoch in range(args.epochs):
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        train_loss += loss.data.item() * batch.batch_size#train_loss += loss.data[0] * batch.batch_size
